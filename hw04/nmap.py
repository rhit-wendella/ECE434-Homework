...
GPIO1_size = 0x44e07fff-GPIO1_offset
GPIO2_offset = 0x4804c000
GPIO2_size = 0x4804cfff-GPIO1_offset
GPIO_OE = 0x134
...
BUTTON1_LINE = 12
BUTTON2_LINE = 13
BUTTON1 = 1<<BUTTON1_LINE
BUTTON2 = 1<<BUTTON2_LINE
...
packed_reg1 = mem[GPIO_OE:GPIO_OE+4]
packed_reg2 = mem[GPIO_OE:GPIO_OE+4]
...
reg_status1 &= ~(LEFT_LED)
reg_status1 &= ~(RIGHT_LED)
...
mem[GPIO_OE:GPIO_OE+4] = struct.pack("<L", reg_status1)
...
try:
  while(True):
    ...
    if(struct.unpack("<L", mem2[GPIO_DATAIN:GPIO_DATAIN+4])[0] & BUTTON1):
      mem[GPIO_SETDATAOUT:GPIO_SETDATAOUT+4] = struct.pack("<L", LEFT_LED)
      time.sleep(0.5)

    if(struct.unpack("<L", mem2[GPIO_DATAIN:GPIO_DATAIN+4])[0] & BUTTON2):
      mem[GPIO_SETDATAOUT:GPIO_SETDATAOUT+4] = struct.pack("<L", RIGHT_LED)
      time.sleep(0.5)
    
    mem[GPIO_CLEARDATAOUT:GPIO_CLEARDATAOUT+4] = struct.pack("<L", LEFT_LED)
    mem[GPIO_CLEARDATAOUT:GPIO_CLEARDATAOUT+4] = struct.pack("<L", RIGHT_LED)
